""" This script provides a way to create individual repos for each of the items in the table """

from datetime import datetime, timedelta, timezone
from optparse import Option
from typing import List, Optional, Dict, Any
from sqlalchemy.orm import Session
from .models import YouTubeVideo, OpenAIArticle, AnthropicArticles, Digest
...
class Repository:
    ...
    def create_anthropic_article(self, guid: str, title: str, url: str, published_at: datetime,
                                description: str = "", category: Optional[str] = None) -> AnthropicArticles:
        existing = self.session.query(AnthropicArticles).filter_by(guid=guid).first()
        if existing:
            return None
        article = AnthropicArticles(
            guid=guid,
            title=title,
            url=url,
            published_at=published_at,
    ...
    def bulk_create_anthropic_articles(self, articles: List[dict]) -> int:
        new_articles = []
        for a in articles:
            existing = self.session.query(AnthropicArticles).filter_by(guid=a["guid"]).first()
            if not existing:
                new_articles.append(AnthropicArticles(
                    guid=a["guid"],
                    title=a["title"],
                    url=a["url"],
                    published_at=a["published_at"],
    ...
    # Create a function to get those anthropic articles that dont have a markdown
    def get_anthropic_articles_without_markdown(self, limit: Optional[int] = None) -> List[AnthropicArticles]:
        query = self.session.query(AnthropicArticles).filter(AnthropicArticles.markdown.is_(None))
        if limit:
            query = query.limit(limit)
        return query.all()

    def update_anthropic_article_markdown(self, guid: str, markdown: str) -> bool:
        article = self.session.query(AnthropicArticles).filter_by(guid=guid).first()
        if article:
    ...
    def get_youtube_video_without_transcript(self, limit: Optional[int] = None) -> List[YouTubeVideo]:
    ...
    def get_articles_without_digest(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        anthropic_articles = self.session.query(AnthropicArticles).filter(
            AnthropicArticles.markdown.isnot(None)
        ).all()
    ...
    def create_digest(self, article_type: str, article_id: str, url: str, 
                    title: str, summary: str, published_at: Optional[datetime] = None) -> Optional[Digest]:
    ...
    def get_recent_digest(self, hours: int = 24) -> List[Dict[str, Any]]:
