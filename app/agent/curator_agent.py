...
# Creating the article ranking criteria
...
# Creating the ranked digest list
class RankedDigestList(BaseModel):
    articles: List[RankedArticle] = Field(description="List of the ranked articles")

# Defining the curator prompt to rerank the articles
...
# creating the curator agent class
class CuratorAgent:
    def __init__(self, user_profile: dict):
        self.client = OpenAI(api_key=os.getenv("OPENAI_API_KEY")) # passing the API Key
        self.model = "gpt-4o-mini" # defining the model
        self.user_profile = user_profile # creating a user profile object
        self.system_prompt = self._build_system_prompt() # utilizing the prompt building function

    # creating the prompt building function
    def _build_system_prompt(self) -> str:
        interests = "\n".join(f" - {interest}" for interest in self.user_profile["interests"]) # getting the interests from the user profile
        preferences = self.user_profile["preferences"] # getting the preferences from the user profile
        pref_text = "\n".join(f" - {k}: {v}" for k, v in preferences.items())

        # Structuring the curator prompt
...
{preferences}"""
    ...
    # defining function for ranking the articles from digest
    def rank_digests(self, digests: List[dict]) -> List[RankedArticle]:
        if not digests: # if there are no articles in digest return an empty list
            return []
        
        # creating a digest list based on the articles in digest in a specific format
        digest_list = "\n\n".join([
            f"ID: {d['id']}\nTitle: {d['title']}\nSummary: {d['summary']}\nType: {d['article_type']}"
            for d in digests
        ])

        # defining the structure of the user_prompt
...
Provide a relevance score (0.0-10.0) and rank (1-{len(digests)}) for each article, ordered from most to least relevant."""
        ...
        # defining the logic and configuration for the API response
        try:
            ...
            ranked_list = response.output_parsed # storing the ranked list as an object 
